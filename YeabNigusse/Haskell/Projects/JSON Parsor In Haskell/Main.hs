...
data JsonValue
  = JsonNull {--this are values a json value has--}
  | JsonBool Bool
  | JsonNumber Integer
  | JsonString String
  | JsonArray [JsonValue]
...
instance Show a => Show (Parser a) where
  show (Parser p) = "Parser p"

instance Functor Parser where
  fmap f (Parser p) = Parser $ \input -> do
    -- to be applicative Parser should be a Functor type class
...
instance Applicative Parser where
  ...
  (Parser p1) <*> (Parser p2) =
    -- to chain the result of each Parser type the parser should implement
    Parser $ \input -> do
      -- Applicatives
      (input', f) <- p1 input
      (input'', a) <- p2 input'
...
jsonBool = f <$> (stringP "true" <|> stringP "false")
  where
    f "true" = JsonBool True -- used to Bool value by implementing Alternatives
...
jsonArray =
  JsonArray
    <$> ( charP '['
            *> ws
            *> elements -- a little bit complicated to explain but in general
            -- used to Parse array of strings into JsonValue
            <* ws
            <* charP ']'
        )
  where
...
parserFile fileName parser = do
  -- used to test the parser from the file
