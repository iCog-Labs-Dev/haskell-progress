...
relevantError [] = []
relevantError ((LogMessage tp@(Error x) _ str) : xs)
  | x > 50 = LogMessage tp x str : relevantError xs
  | otherwise = relevantError xs
relevantError ((LogMessage tp _ str) : xs) = relevantError xs
